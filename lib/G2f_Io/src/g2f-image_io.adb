...
package body G2f.Image_Io is
   ...
   -- image_info
   function Clone_Image_Info (Image_Info_In: in Image_Info_Ptr) return Image_Info_Ptr is
      function C_CloneImageInfo (Image_Info_In: in Image_Info_Ptr) return Image_Info_Ptr;
...
--      Free(Image_Info_In);
   ...
   function Read_Image (I:in Image_Info_Ptr) return Image_Ptr is
      function C_Read_Image (I:in Image_Info_Ptr;
   ...
   begin
      ...
      Put_Line ("Size PixelPacket = " & Integer'Image (Pixel_Packet'Size/8));
      Mon_Image:=C_Read_Image(I);
      if Mon_Image = null then
         raise Read_Image_Error;
      end if;
      Set_Compression(Mon_Image,NoCompression); --the user may explicitly give a compression methode.
      return Mon_Image;
   end Read_Image;
   ...
   procedure Write_Image(I:in Image_Info_Ptr;
      ...
      function C_Write_Image(I:in Image_Info_Ptr;
                             E:in Image_Ptr) return Unsigned;
      pragma Import (C,C_Write_Image,"WriteImage");
      Res:Unsigned:=0;
   begin
      Res:=C_Write_Image(I,E);
      --if Res = 0 or E.all.Image_Exception.Reason /= interfaces.c.strings.Null_Ptr then
   ...
   function Ping_Image (I:in Image_Info_Ptr) return Image_Ptr is
      function C_Ping_Image (I:in Image_Info_Ptr;
                             E:in Exception_Info_Ptr:=Ex_Info_Ptr) return Image_Ptr;
