...
package body G2f.Io is
   ...
   function To_Magick_Format (Format: in Supported_Image_Formats) return String is
   ...
   procedure Set_Format(I:in Image_Info_Ptr; Format: in Supported_Image_Formats) is
   ...
   begin
      for I in Name'Range loop
         if Name(I) = ':' then Prefix:=True;
         elsif Name(I) = '.' then Suffix:=True;
         end if;
      end loop;
      if Prefix=False and then Suffix=True then
         I.all.Filename(Size_T(Res'First)..Size_T(Res'Last+1+Name'Last+1)):=To_C(Res&':'&Name);
   ...
   procedure Set_Format(I:in Image_Ptr; Format: in Supported_Image_Formats) is
      Res:String:=To_Magick_Format (Format);
   begin
      I.all.Magick(Size_T(Res'First)..Size_T(Res'Last+1)):=To_C(Res);
   end Set_Format;
   ...
   procedure Set_Compression (I:in Image_Info_Ptr;C: in Compression_Type) is
   begin
      I.all.Compression:=C;
   end Set_Compression;
   ...
   procedure Set_Depth  (I:in Image_Ptr; D: in Depth) is
      function C_Set_Image_Depth (Image:Image_Ptr;Depth:C.Unsigned_Long) return C.Int;
      pragma Import (C,C_Set_Image_Depth,"SetImageDepth");
      Res:C.Int:=0;
   begin
      Res:=C_Set_Image_Depth(I,C.Unsigned_Long(D));
      if Res = 0 then raise Depth_Error;
      end if;
   end Set_Depth;
   ...
   procedure Set_Image_Size (I:in Image_Info_Ptr; Im_S:in Image_Size) is
      ...
      Str_X_Y:String:=(Str_X(Str_X'First+1..Str_X'Last) & 'x' & Str_Y(Str_Y'First+1..Str_Y'Last));
