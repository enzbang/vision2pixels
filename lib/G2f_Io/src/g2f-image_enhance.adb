...
package body G2f.Image_Enhance is
   ...
   procedure Contrast_Image (I: in out Image_Ptr; Sharpen:in Sharpen_T; Factor:in Positive:=1) is
      use C;
      Res:C.Int:=0;
      S:C.Unsigned:=0;
      function C_Contrast_Image(I: in Image_Ptr; Sharp: in C.Unsigned) return C.Int;
   ...
   procedure Gamma_Image (I:in out Image_Ptr; Red,Green,Blue:in Gamma_Parameter:=1.0) is
      use C;
      Res:C.Int:=0;
      function C_Gamma_Image(I: in Image_Ptr;G: in C.Char_array) return C.Int;
      pragma Import (C,C_Gamma_Image,"GammaImage");
   begin
      Res:=C_Gamma_Image(I,To_C(Gamma_Parameter'Image(Red)&'/'&
   ...
   procedure Level_Image (I:in out Image_Ptr; Black_Percent,White_Percent:in Persent_Level;Gamma:in Gamma_Parameter:=1.0) is
      use C;
      Res:C.Int:=0;
      function C_Level_Image(I: in Image_Ptr;Level: in C.Char_Array) return C.Int;
      pragma Import (C,C_Level_Image,"LevelImage");
   begin
      Res:=C_Level_Image(I,To_C(Persent_Level'Image(Black_Percent)&"%"&','&
   ...
   procedure Level_Image_Channel (I:in out Image_Ptr;
                                  Channel:in Channel_T;
                                  Black_Point,White_Point:in Max_Rgb;
                                  Gamma:in Gamma_Parameter:=1.0) is
      use C;
      Res:C.Int:=0;
      function C_Level_Image_Channel(I: in Image_Ptr;
                                     Channel: Channel_T;
                                     B_P,W_P: in Max_Rgb;
                                     G: in Gamma_Parameter) return C.Int;
      pragma Import (C,C_Level_Image_Channel,"LevelImageChannel");
   begin
      Res:=C_Level_Image_Channel(I,
                                 Channel,
   ...
   procedure Modulate_Image (I:in out Image_Ptr; Brightness,Saturation,Hue:in Persent_Level) is
      use C;
      Res:C.Int:=0;
      function C_Modulate_Image(I: in Image_Ptr;Level: in C.Char_Array) return C.Int;
      pragma Import (C,C_Modulate_Image,"ModulateImage");
   begin
      Res:=C_Modulate_Image(I,To_C(Persent_Level'Image(Brightness)&','&
   ...
   procedure Negate_Image (I:in out Image_Ptr; Only_Grayscale: in Boolean:=False) is
      use C;
      Res:C.Int:=0;
      function C_Negate_Image(I: in Image_Ptr; O_G:in C.Unsigned) return C.Int;
